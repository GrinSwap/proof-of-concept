...
from grin.util import UUID, absolute, MILLI_GRIN_UNIT, OrderedEnum
...
class AtomicSwap:
    def __init__(self, secp: Secp256k1, role: Role, id: UUID):
        ...
        # Default values
        self.stage = self.wallet = self.grin_amount = self.eth_amount = self.eth_receive_address = self.lock_height \
            ...
            = self.foreign_public_nonce = self.foreign_public_refund_nonce = self.eth_secret_lock \
            = self.eth_public_lock = self.eth_address_lock = self.eth_contract_address = self.commit \
            ...
            = self.tx_height = self.swap_nonce = self.public_swap_nonce = self.foreign_public_swap_nonce \
            = self.swap_entry = self.swap_child = self.swap_fee_amount = self.swap_lock_height = self.swap_output \
            = self.swap_offset = self.public_swap_excess = self.partial_swap_signature \
            = self.partial_swap_signature_extra = self.foreign_partial_swap_signature_extra \
            = self.foreign_partial_swap_signature = self.swap_tx = self.claim = None
    ...
    def fill_signatures(self):
        ...
        self.partial_signature = aggsig.calculate_partial(
            self.secp, excess, self.nonce, public_nonce_sum, self.fee_amount, self.lock_height
        )

        # First step of multi party bullet proof
        proof_builder = MultiPartyBulletProof(self.secp, self.partial_child, self.foreign_partial_commit.to_public_key(
            self.secp), self.grin_amount, self.commit)
        self.t_1, self.t_2 = proof_builder.step_1()

        if seller:
            proof_builder.fill_step_1(self.foreign_t_1, self.foreign_t_2)
            self.tau_x = proof_builder.step_2()
        ...
        self.partial_refund_signature = aggsig.calculate_partial(
            self.secp, refund_excess, self.refund_nonce, public_refund_nonce_sum, self.refund_fee_amount,
    ...
    def finalize_range_proof(self):
        ...
        proof_builder = MultiPartyBulletProof(self.secp, self.partial_child, self.foreign_partial_commit.to_public_key(
            self.secp), self.grin_amount, self.commit)
        proof_builder.fill_step_1(self.foreign_t_1, self.foreign_t_2)
        proof_builder.fill_step_2(self.foreign_tau_x)
    ...
    def fill_swap_signatures(self):
        ...
        if not buyer:
            ...
            assert aggsig.verify_partial_extra(self.secp, self.foreign_partial_swap_signature_extra,
                                               public_swap_nonce_sum, foreign_public_partial_swap_excess,
                                               self.eth_public_lock, self.swap_fee_amount, self.swap_lock_height), \
                "Partial swap signature not valid"

        # Partial swap signature
        self.partial_swap_signature = aggsig.calculate_partial(
            self.secp, swap_excess, self.swap_nonce, public_swap_nonce_sum, self.swap_fee_amount,
            self.swap_lock_height
        )

        if buyer:
            self.partial_swap_signature_extra = aggsig.calculate_partial_extra(
                self.secp, swap_excess, self.swap_nonce, self.eth_secret_lock, public_swap_nonce_sum,
    ...
    def finalize_swap(self):
        ...
        if seller:
            self.eth_secret_lock = self.foreign_partial_swap_signature_extra.scalar(self.secp).add(
                self.secp, self.foreign_partial_swap_signature.scalar(self.secp).negate(self.secp)
            )

            eth_public_lock = self.eth_secret_lock.to_public_key(self.secp)
            assert self.eth_public_lock == eth_public_lock, "Invalid secret lock!"

            self.claim = self.secp.sign_recoverable(self.eth_secret_lock, bytearray([0] * 32))
    ...
    def load(self, dct=None):
        ...
        self.eth_amount = int(dct['eth_amount'])
        self.eth_receive_address = dct['eth_receive_address']
        ...
        if seller:
            ...
            if from_file:
                self.eth_secret_lock = SecretKey.from_hex(self.secp, dct['eth_secret_lock'].encode())
        ...
        if self.stage >= Stage.SIGN or buyer:
            ...
            if from_file:
                self.eth_public_lock = PublicKey.from_hex(self.secp, dct['eth_public_lock'].encode())
            self.eth_address_lock = ethereum_address(self.secp, self.eth_public_lock).decode()
            self.commit = self.secp.commit_sum([self.foreign_partial_commit,
                                                self.wallet.commit(self.partial_entry)], []) \
                if not from_file else Commitment.from_hex(self.secp, dct['commit'].encode())
        ...
        if self.stage >= Stage.LOCK:
            ...
            if buyer:
                ...
                self.partial_swap_signature_extra = Signature.from_hex(dct['partial_swap_signature_extra'].encode())
        ...
        if seller and self.stage >= Stage.SWAP:
            self.foreign_partial_swap_signature_extra = Signature.from_hex(
                dct['foreign_partial_swap_signature_extra'].encode()
            )
    ...
    def to_dict(self, all: bool):
        ...
        if all or (self.stage == Stage.INIT and seller):
            dct.update({
                ...
                "eth_receive_address": self.eth_receive_address,
        ...
        if all:
            ...
            if seller:
                ...
                dct.update({
                    "eth_secret_lock": self.eth_secret_lock.to_hex().decode()
                })

            if self.stage > Stage.INIT or buyer:
                dct.update({
        ...
        if (all and (self.stage > Stage.INIT or buyer)) or (self.stage == Stage.INIT and buyer):
            dct.update({
                "eth_public_lock": self.eth_public_lock.to_hex(self.secp).decode(),
        ...
        if (all and (self.stage > Stage.INIT or buyer)) or (self.stage == Stage.INIT and buyer) or \
        ...
        if seller and ((all and self.stage > Stage.INIT) or self.stage == Stage.SIGN):
            dct.update({
                "tau_x": self.tau_x.to_hex().decode()
            })

        if (all and ((self.stage > Stage.INIT and buyer) or (self.stage > Stage.SIGN and seller))) or \
        ...
        if buyer and ((all and self.stage >= Stage.LOCK) or self.stage == Stage.LOCK):
            dct.update({
                "partial_swap_signature_extra": self.partial_swap_signature_extra.to_hex().decode()
            })

        if all and self.stage >= Stage.SWAP and seller:
            dct.update({
                "foreign_partial_swap_signature_extra": self.foreign_partial_swap_signature_extra.to_hex().decode()
            })
    ...
    def receive(self, dct: dict):
        if self.stage is None:
            if self.role == Role.BUYER:
                ...
                self.eth_secret_lock = SecretKey.random(self.secp)
                self.eth_public_lock = self.eth_secret_lock.to_public_key(self.secp)
            ...
            if self.role == Role.SELLER:
                ...
                self.swap_output = Output.from_dict(self.secp, dct['swap_output'], True)
                self.swap_offset = BlindingFactor.from_hex(dct['swap_offset'].encode())
                self.foreign_partial_swap_signature_extra = Signature.from_hex(dct['partial_swap_signature_extra'])
            else:
